...
## Phase v0.6 — échelle du corpus et de la vue — **SPECS ÉCRITES le 20/09 (change `scale-corpus`, documentaire, resserrées sur revue du soir)**

Contrainte posée par l'utilisateur : le projet doit tourner ailleurs — corpus 20-100× plus lourd au minimum, base source PC de 4 Go. Vérification code : la recherche tient (SQL pur FTS5 — à confirmer au banc) ; `readJsonl` plein-fichier, l'ingestion à réécriture intégrale et `raw/` plat cassent à l'échelle.

- ✅ Specs poussées (`scale-corpus`) : layout v2 éclaté en shards par session (enregistrements inchangés ; **préfixe de répartition par condensat déterministe** — les id partagent `ses_`/`prt_`, un préfixe tiré des premiers caractères ne répartirait rien) ; coût de passe **honnête** (delta source + sessions touchées + métadonnées, watermark en requête indexée — jamais de parcours complet) ; **protocole de publication** (marqueur d'ingestion en cours posé avant tout remplacement et retiré en dernier, staging, renames, COMMIT de la vue = point de publication, `state.json`, GC des temporaires ; jamais de shard déchiré, publication entre shards éventuelle, lectures toujours via la vue dans une **transaction de lecture unique** ; opérations d'archive refusées tant que non réconcilié, réconciliation par relance, reprise explicite sans source) ; **vue dérivable SQLite en chemin de lecture unique** (JSON intégral + métadonnées, fenêtres bornées, compteurs par dénombrement de plage mesuré, fraîcheur limitée au chemin d'ingestion — hors-ingestion détecté par l'empreinte) ; **mémoire bornée partout** (streaming, preuves par blocs) ; migration v1→v2 sans source ; empreinte déterministe du corpus ; cibles re-visées au **banc synthétique 100× mesurant le parcours complet** (recherche rendue + `--ctx`, `read --at` + compteurs, ingestion depuis base source à structure opencode réelle, preuve volumineuse ; machine/cache/RSS/disque consignés).
...
- ✅ **IMPLÉMENTÉE le 20/09 (commit `6104bc9`, +2 144 lignes / 20 fichiers)** — vérification reprise en main le même soir : tests **92/92 en ~7 s** (reproduits 4 fois, y compris avec la ligne de commande complète du harnais qui avait bouclé — le blocage était un enfant de test orphelin du pipeline `node --test | grep | head` du harnais, pas le code) ; corpus réel migré et vérifié (empreinte `ccdf5b…` sur 6 441 fichiers, `read --at` revalidé sur la session n46 : 20 visibles / 38 masqués) ; banc consigné README (20k : p95 85 ms, RSS 138 Mo ; 100k : p95 167 ms — cible 100 ms @500k à re-mesurer sur machine cible). Dorées : 28/28 sur corpus figé pré-delta ; **24/28 sur corpus vivant** — les 4 écarts viennent des sessions d'examen `eval-run` (delta 19/09) qui citent les requêtes dorées verbatim et gagnent le BM25 — à arbitrer (exclusion, dorées mises à jour, ou écart documenté).
