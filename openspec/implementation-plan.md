...
- ✅ Specs poussées (`scale-corpus`) : layout v2 éclaté en shards par session (enregistrements inchangés ; **préfixe de répartition par condensat déterministe** — les id partagent `ses_`/`prt_`, un préfixe tiré des premiers caractères ne répartirait rien) ; coût de passe **honnête** (delta source + sessions touchées + métadonnées, watermark en requête indexée — jamais de parcours complet) ; **protocole de publication** (staging, renames, COMMIT de la vue = point de publication, `state.json` en dernier, GC des temporaires ; jamais de shard déchiré, publication entre shards éventuelle, lectures toujours via la vue) ; **vue dérivable SQLite en chemin de lecture unique** (JSON intégral + métadonnées, fenêtres bornées, compteurs par dénombrement de plage mesuré, fraîcheur limitée au chemin d'ingestion — hors-ingestion détecté par l'empreinte) ; **mémoire bornée partout** (streaming, preuves par blocs) ; migration v1→v2 sans source ; empreinte déterministe du corpus ; cibles re-visées au **banc synthétique 100× mesurant le parcours complet** (recherche rendue + `--ctx`, `read --at` + compteurs, ingestion depuis base source à structure opencode réelle, preuve volumineuse ; machine/cache/RSS/disque consignés).
